...
function deduplicateBusinesses(businesses: Business[]): Business[] {
  ...
  console.log(
    `Deduplication: ${businesses.length} -> ${deduplicated.length} businesses`
  )
  return deduplicated
}
...
async function searchNearbyBusinesses(
  ...
  // Build request body - when using pageToken, we must send it with all original params
  const requestBody: Record<string, unknown> = {
    ...
    rankPreference: rankPreference.toUpperCase(), // Convert to uppercase: DISTANCE or PROMINENCE
  }

  // Add pageToken if this is a pagination request
  if (pageToken) {
  ...
  const response = await fetch(url, {
    ...
    headers: {
      ...
      'X-Goog-FieldMask':
        'places.displayName,places.formattedAddress,places.location,places.id,places.types,places.rating,places.userRatingCount,places.nationalPhoneNumber,places.websiteUri',
    },
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Places API request failed: ${response.status} - ${errorText}`
    )
  }
  ...
  console.log(
    `Website status: ${withWebsite} with website, ${withoutWebsite} without website`
  )
  ...
  if (excludeLargeBusinesses) {
    ...
    filtered = filtered.filter(
      (b) => !b.reviewCount || b.reviewCount < maxReviewCount
    )
    ...
    filtered = filtered.filter((b) => {
      ...
      return !chainBlocklist.some((chain) =>
    ...
    console.log(
      `Large business filtering: excluded ${excludedByReviewsCount} by reviews, ${excludedByBlocklistCount} by blocklist, ${excludedByTypeCount} by type`
    )
  }
  ...
  if (filterNoWebsite) {
    ...
    console.log(
      `Filtering businesses without website: ${beforeWebsiteFilter} -> ${filtered.length}`
    )
  }
  ...
  console.log(
    `Search results: scannedCount=${scannedCount}, returnedCount=${returnedCount}, filterNoWebsite=${filterNoWebsite}, excludeLargeBusinesses=${excludeLargeBusinesses}, hasNextPage=${!!data.nextPageToken}, nextPageToken=${data.nextPageToken || 'NONE'}`
  )
